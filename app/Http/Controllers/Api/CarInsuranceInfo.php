...
class CarInsuranceInfo extends Controller
{
    ...
    public function carInsuranceInfo(CarInsuranceInfoRequest $request) {
        try {
            ...
            $data = $request->only(['carrier', 'policy_number', 'agent_name','car_detail_id']);
            $data['user_id'] = $userId;
    ...
    public function getCarInsurenceInfoUsingCarDetailsId() {
        try {
            $carDetailsId =request()->input('car_detail_id');
            if(!$carDetailsId) {
                return response()->json([
                    'status'=>false,
                    'message'=>"Car Detail ID is required",
                ],400);
            }
            $carInsuranceInfo = $this->carInsuranceInfoRepository->getCarInsurenceInfoUsingCarDetailsId($carDetailsId);
            if(!$carInsuranceInfo) {
                return response()->json([
                    'status'=>false,
                    'message'=>"Car insurance info not found",
    ...
    public function carInsurenceInfoUpdate(CarInsuranceInfoRequest $request) {
         try {
            $userId = auth()->user()?auth()->user()->id:$request->input('user_id');
            $data = $request->only(['carrier', 'policy_number', 'agent_name','car_detail_id']);
            $data['user_id'] = $userId;
